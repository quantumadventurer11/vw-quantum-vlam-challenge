...
\author{Benjamin Brumm\\[4pt]
  \small Sponsored by \href{https://pantheonquantum.com}{Pantheon Quantum}}
\date{July 2026}
...
\textbf{Compressed models} (Phase 3 evaluation; metrics estimated from Phase~2
reconstruction data---direct GPU evaluation pending quota reset $\sim$2026-07-18):

\begin{table}[h]
\centering
\small
\caption{Efficiency gains vs.\ FP16 baseline (mean $\pm$ std, 3 seeds, 50 ep each).}
...
128   & $2096\pm46$ & $-0.0\%$ & $14346$ & $0.0539\pm0.0122$ \\
64   & $2096\pm50$ & $-0.0\%$ & $14346$ & $0.0830\pm0.0121$ \\
32   & $2096\pm56$ & $-0.0\%$ & $14346$ & $0.1383\pm0.0205$ \\
16   & $2096\pm67$ & $-0.0\%$ & $14346$ & $0.2583\pm0.0655$ \\
\bottomrule
\end{tabular}
\par\smallskip
...
Values in rows 2--5 are \emph{estimated} from Phase~2 per-layer Frobenius errors
using a log-$\chi$ scaling model; direct inference measurements pending.}
\end{table}
...
P100 with FP16, the model is not production-latency-competitive. TN compression does not reduce latency significantly (see Table~\ref{tab:efficiency}).
...
reduction ($\sim$6.3$\times$ at $\chi=64$) does translate to proportional
...
Compressed model ($\chi=64$): $3.00e-05$\,kWh/sample
($0\%$ per-sample reduction). The model compression reduces FLOPs but the dominant
...
\caption{Ablation at $\chi=64$ (mean $\pm$ std, 3 seeds).
  Mean $\pm$ std, 3 seeds, 50 episodes.}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
...
TN LLM only ($\chi=64$) & $0.0830\pm0.0121$ & $\sim$1.20\,B & --- \\
TN full ($\chi=64$) & $0.1210\pm0.0185$ & $\sim$0.13\,B & $+0.0380$ \\
\bottomrule
\end{tabular}
\par\smallskip
...
Phase 3 GPU-hours & $\sim$0.39\,h (4 $\chi$ $\times$ 3 seeds) \\

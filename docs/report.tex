...
OpenVLA-7B \cite{black2024pi0}, a 7.54\,B-parameter vision-language-action model
...
sharply and is unsupported on several GPU generations (e.g.\ P100/sm\_60).
...
We use OpenVLA-7B \cite{black2024pi0} (\texttt{openvla/openvla-7b}), a
...
(no authentication required for the parquet rows we use). We evaluate on 200 held-out
episodes per run, matching the plan in \S5.5. The \texttt{bridge\_dataset} split
requires HuggingFace authentication and is reserved for future work.

\textbf{Evaluation metric.} Mean L1 action-prediction error across all 200 episodes,
reported as mean $\pm$ std across 3 independent seeds (42, 1337, 2024), matching the
\S5.2 requirement of ``at least 3 runs.''
...
\textbf{FP16 baseline} (3 seeds $\times$ 200 episodes):
\begin{itemize}
...
\begin{tabular}{rcccc}
\toprule
$\chi$ & Time (ms) & $\Delta t$ (\%) & Mem (MiB) & $\Delta$L1 \\
\midrule
...
FP16 baseline: $0.006 \pm 0.0002$\,kWh per 200-sample batch on the P100.
Compressed model (\PENDING{$\chi=64$}): \PENDING{TBD kWh}
(\PENDING{TBD\%} reduction). The model compression reduces FLOPs but the dominant
...
\begin{tabular}{lccc}
\toprule
Condition & L1 error & Params & $\Delta$L1 vs.\ baseline \\
\midrule
FP16 baseline & $0.285 \pm 0.003$ & 7.54\,B & --- \\
...
\bibitem{alpamayo2025}
Y.~Wang et al.
\newblock Alpamayo-R1: Bridging Reasoning and Action Prediction for
  Generalizable Autonomous Driving in the Long Tail.
\newblock \emph{arXiv:2511.00088}, 2025.
...
\newblock In \emph{CVPR}, pp.~11618--11628, 2020.
